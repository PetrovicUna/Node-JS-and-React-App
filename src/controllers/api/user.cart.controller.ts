...
export class UserCartController {
    ...
    private async getActiveCartForUserId(userId: number): Promise<Cart>{
        ...
        let cart = await this.cartService.getListCartByUserId(userId);
    ...
    //GET https://localhost:3000/api/user/cart/
    @Get() 
    ...
    //POST https://localhost:3000/api/user/cart/addToCart/
    @Post('addToCart')
    @UseGuards(RoleCheckedGuard)
    @AllowToRoles('user')
    ...
    //PATCH https://localhost:3000/api/user/cart/
    @Patch()
    @UseGuards(RoleCheckedGuard)
    @AllowToRoles('user')
    ...
    //POST https://localhost:3000/api/user/cart/makeOrder/
    @Post('makeOrder')
    @UseGuards(RoleCheckedGuard)
    @AllowToRoles('user')
