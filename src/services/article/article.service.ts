...
export class ArticleService extends TypeOrmCrudService<Article>{
    ...
    async createFullArticle(data: AddArticleDto): Promise<Article | ApiResponse> {
        ...
        if(!savedArticle){
            return new ApiResponse('error', -5001, 'Could not save new article data.');
        }
        ...
        let savedArticlePrice = await this.articlePrice.save(newArticlePrice);

        if(!savedArticlePrice){
            return new ApiResponse('error', -5001, 'Could not save new article price data.');
        }
        ...
        for(let feature of data.features){
            ...
            let savedArticleFeature = await this.articleFeature.save(newArticleFeature);

            if(!savedArticleFeature){
                return new ApiResponse('error', -5001, 'Could not save new article feature data.');
            }
        }
    ...
    async editFullArticle(articleId: number, data: EditArticleDto): Promise<Article | ApiResponse>{
        ...
        if(newPriceString !== lastPriceString){
            ...
            if(!savedArticlePrice) {
                return new ApiResponse('error', -5001, 'Could not save new article price.');
            }
        }
