...
export class OrderService {
    ...
    async add(cartId: number): Promise<Order | ApiResponse> {
        ...
        const cart = this.cart.findOne(cartId, {
            relations: [
                "cartArticles",
        ...
        if(!cart){
            return new ApiResponse("error", -7002, "Not such a cart found!");
        }

        if((await cart).cartArticles.length === 0){
    ...
    async changeStatus(orderId: number, newStatus: "rejected" | "accepted" | "shipped" | "pending"){
        ...
        if(!order){
            return new ApiResponse("error", -9001, "Not such order found!")
        }
